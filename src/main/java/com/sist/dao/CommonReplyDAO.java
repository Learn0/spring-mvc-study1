...
public class CommonReplyDAO {
	...
	// 1. 목록 출력
	/*
	 * CREATE OR REPLACE PROCEDURE replyListData( pCno commonReply.cno%TYPE, pTno
	 * commonReply.tno%TYPE, pResult OUT SYS_REFCURSOR ) IS BEGIN -- cursor
	 * (ResultSet) => select문장 실행시 출력된 모든 데이터를 읽어 온다 -- 목록을 출력(여러개의 데이터를 모아서 넘겨주는 경우
	 * : CURSOR OPEN pResult FOR SELECT
	 * no,cno,tno,id,name,msg,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') FROM
	 * commonReply WHERE cno=pCno AND tno=pTno;
	 * 
	 ...
	 */
	/*
	 * 클래스마다 역할 분담 ============== ~Controller : 데이터베이스 연결해서 결과값을 가지고 와서 JSP로 전송 ~VO
	 * : 데이터값만 저장 (전송목적) ~DAO : 데이터베이스만 연결하는 역할
	 * 
	...
	// 2. 댓글 쓰기
	/*
	 * CREATE OR REPLACE PROCEDURE replyInsert( pCno commonReply.cno%TYPE, --
	 * commonReply.cno%TYPE cno NUMBER pTno commonReply.tno%TYPE, pId
	 * commonReply.id%TYPE, -- id VARCHAR2(20) pName commonReply.name%TYPE, -- name
	 * VARCHAR2(34) pMsg commonReply.msg%TYPE -- msg CLOB ) IS BEGIN INSERT INTO
	 * commonReply VALUES( (SELECT NVL(MAX(no)+1,1) FROM commonReply),
	 * pCno,pTno,pId,pName,pMsg,SYSDATE ); COMMIT; END; /
	 */
	...
	// 3. 댓글 수정
	/*
	 * CREATE OR REPLACE PROCEDURE replyUpdate( pNo commonReply.no%TYPE, pMsg
	 * commonReply.msg%TYPE ) IS -- 선언부 BEGIN -- 구현부 UPDATE commonReply SET msg=pMsg
	 * WHERE no=pNo; COMMIT; END; /
	 */
	...
	// 4. 댓글 삭제
	/*
	 * CREATE OR REPLACE PROCEDURE replyDelete( pNo commonReply.no%TYPE ) IS BEGIN
	 * DELETE FROM commonReply WHERE no=pNo; COMMIT; END; /
	 */
