...
public class FoodDAO {
	...
	/*
	 * ============================ 유지보수 주문 내역 확인 (HandlerMapping)
	 * 
	 * @RequestMapping
	 * 
	 * @GetMapping
	 * 
	 * @PostMapping ========= 미리 제작 요청 (category.do) ======> DispatcherServlet
	 * =======> 주문 내역을 만든다(@Controller)<==> 재료 맛집 카테고리를 보여 달라 서빙(주문) =>
	 * 배달 @RestController ~Manager,~DAO Dispatcher:배달부 FilterDispatcher DAO
	 * => @Controller,@RestController ====> 라이더(ViewResolver) ====> 요청 브라우저로 전송
	 * 
	 * 개발 : 8개월 , 4개월 : 유지보수 SM/SI (80%), 솔루션 : 자체 개발 (기술영업 , SM) 프레임워크 : SM(X) =>
	 * 개발 (위탁개발)
	 * 
